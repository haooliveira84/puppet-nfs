...
def mk_client_list(clients, options, default, _err)
  # Make clients into an array if it isn't already.
  unless clients.is_a?(Array)
    clients = [clients]
  end
  ...
  clients.each do |client|
    client_options = if options.is_a?(Hash) && options.key?(client)
                       options[client]
                     else
                       default
                     end
